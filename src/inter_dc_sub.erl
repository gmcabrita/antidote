...
connect_to_node([Address|Rest]) ->
	...
	{ok, _} ->
	    ...
	    lists:foreach(fun(P) ->
				  %% Make the socket subscribe to messages prefixed with the given dcid and partition number
				  ok = zmq_utils:sub_filter(Socket, inter_dc_txn:dcid_and_partition_to_bin(MyDcId, P))
